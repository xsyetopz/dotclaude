...
<output>
For each finding: what happened, the root cause, the action (with the file and the exact text or change), and its priority. End with a one-line takeaway. Do not apply any action until the user picks which ones to apply, since these change how future sessions behave.
</output>
