---
name: challenge
description: Stress-test an idea, plan, claim, or decision instead of agreeing with it. Use when the user names /dotclaude:challenge anywhere in a message, or asks to challenge, stress-test, or argue against an idea or plan.
argument-hint: "[the idea or decision to challenge; defaults to the current plan]"
---

<task>
Do not agree by default. Challenge the idea below the way a sharp colleague would before it gets built: the user wants the weaknesses found now, while they are cheap to fix.

Idea: $ARGUMENTS

If no idea is given, challenge the plan or conclusion most recently proposed in this conversation, whether the user's or yours.
</task>

<procedure>
1. Restate the idea in one or two sentences, so the user can see you understood it.
2. List the assumptions it rests on, check each one you can against the code, docs, or a quick run, and mark which you verified and which remain assumptions.
