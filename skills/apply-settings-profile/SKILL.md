...
description: Preview and apply the dotclaude settings profile to a Claude Code settings file (fast mode off; Opus 5.5, Fable 5.1, and Haiku 4.5 with the Sonnet alias mapped to Opus; built-in git instructions replaced; secret-file read denies; an xhigh effort cap; subagent and workflow bounds), with an optional managed-settings drop-in the user installs with admin rights, and optionally add a marked dotclaude section to the global CLAUDE.md. Run when the user types /dotclaude:apply-settings-profile or asks to apply, update, or check the dotclaude settings.
...
3. Show the user the listed changes, grouped as below, and ask whether to apply them.
   ...
   | Group | Keys | Why |
   | --- | --- | --- |
   | Fast mode off | `env.CLAUDE_CODE_DISABLE_FAST_MODE=1`, `fastMode: false`, `fastModePerSessionOptIn: true`, `ultracode: false` | The env var removes the `/fast` toggle; the others keep a stray toggle from persisting and keep ultracode's xhigh orchestration from turning on persistently. Together with `workflowKeywordTriggerEnabled: false`, ultracode starts only when the user picks it with `/effort ultracode`; no setting disables it alone, and the `xhigh` effort cap does not block it. |
   ...
   | Effort cap | `maxEffortLevel: xhigh` | `max` is blocked because the claude.ai effort picker warns it uses about 5.5x usage on Opus 5.5 and 3.5x on Fable 5.1 (as of 2026-09-26). `xhigh` stays available for the rare hard turn that needs it. The lowest cap across settings scopes applies. |
   | Subagent and workflow bounds | `env.CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS=6`, `env.CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS=6`, `workflowSizeGuideline: medium` | Bounds fan-out, which is the largest driver of quota burn in long sessions; past about six parallel agents, rate limits make agents retry and burn context. The workflow env var is a hard cap on agents running at once in a workflow; the size guideline is advice to Claude on how many agents to plan, not an enforced limit. Workflows stay enabled. |
   ...
   | Schema | `$schema` | Points editors at the published settings schema, so they validate and complete the file. |
...
<managed_settings>
User and project settings stay editable by the user, so a stray `/fast` or a later edit can undo them. After the profile is applied, offer the managed drop-in as an optional self-lock: a managed settings file the user cannot change or remove without admin rights. It sets `maxEffortLevel: "xhigh"`, `fastMode: false`, `fastModePerSessionOptIn: true`, and the three `availableModels`, and nothing else. Say plainly that undoing it later also takes admin rights, and install it only if the user wants that.
...
2. If the user wants the lock, give them the command the dry run printed, to run in their own terminal, since `sudo` needs a terminal for its password prompt. It names Bun by absolute path because `sudo` resets `PATH` on Linux:
