---
name: polish
description: Find everything a senior perfectionist reviewer would reject in the current change, then fix it. Run when the user types /dotclaude:polish, typically after building a feature.
...
2. List what they would reject, most serious first: incorrect or unhandled cases, weak or missing tests, names that do not match the domain, duplicated logic an existing helper covers, dead code and leftover scaffolding, inconsistent error handling, comments that restate the code or are now wrong, stale docs, and anything that breaks the repository's conventions.
