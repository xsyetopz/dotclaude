---
name: fresh-eyes
description: Have a fresh reviewer, who never saw the authoring reasoning, check a new document, report, or change for errors, omissions, and weak data, then fix what holds up. Run when the user types /dotclaude:fresh-eyes.
disable-model-invocation: true
argument-hint: "[path(s) or git range; defaults to what was just written]"
---

<task>
Re-review the target for errors, omissions, and weak data, then fix all of it. A second or third pass over a new document routinely finds major errors the author missed, even when the author expects none, so run the review even if the work seems right.

Target: $ARGUMENTS (if empty, the document or change most recently written in this conversation).
</task>

<procedure>
1. Review pass. Launch one `general-purpose` agent (not a fork, so it does not inherit your reasoning). Give it only the target path(s) or git range, the user's request that the target serves, quoted verbatim, and this instruction: "Review this with fresh eyes. For every factual claim, command, path, number, and quotation, check it against the repository, the sources it cites, or a run. Report errors, omissions (things the request needs that are missing), weak or unsupported data, and internal contradictions, each with its location and the evidence. Do not edit anything." Add none of your own diagnosis, expectations, or reasoning behind the draft, since that would anchor the reviewer on the author's view.
2. Check the findings. Confirm or reject each against the evidence yourself; reviewers can be wrong too.
